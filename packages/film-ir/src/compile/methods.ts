...
export function forensicMethods(versions: { analyzer: string; opencv: string; pyav: string; numpy: string; ocr: string | null }): Method[] {
  ...
  return [
    ...
    m('boundary.cut', 'vision', 'Hard cut detection', 'A single-frame discontinuity where histogram distance, pixel difference and edge change ratio all exceed adaptive thresholds; one-frame flashes excluded, and the same picture brightening or changing colour (edge change ratio under 0.3 with 70% of tracked features surviving) is not a cut.', { histogramFloor: 0.3, pixelFloor: 0.05, samePictureEdgeChange: 0.3, samePictureTracked: 0.7 }),
    m('boundary.wipe', 'vision', 'Wipe within a frame', 'A cut whose first frame is, block by block (10 px at 160 px wide), explained by the frames either side far better than by either alone — its residual under 40% of the outgoing frame\'s and 80% of the incoming\'s — with 10–90% of it still the outgoing picture: an iris or a wipe completed within one frame.', { blockPx: 10, outgoingShare: [0.1, 0.9] }),
    ...
    m('boundary.dissolve', 'vision', 'Dissolve detection', 'A run of frames fitting α·A + (1−α)·B with A and B the frames either side, α monotonic from ≥0.7 to ≤0.3, mean RMS residual ≤ 0.02, the change spread over two frames or more; widened while the frames fit as well, then trimmed to the frames it mixes. Where A and B keep most of the same edges (edge change ratio under 0.5 at working resolution), it is a cross-fade inside the shot, not a boundary.', { keptStructure: 0.5 }),
    m('visual.crossfade', 'vision', 'Cross-fade inside a shot', 'A dissolve, measured as one, whose two ends keep most of the same edges: part of the picture cross-fades — a background behind type or windows that stay — and the shot goes on.'),
