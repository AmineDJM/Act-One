...
export async function runRevision(
  ...
  options: { storyboardId: string; instruction: string; authorUserId: string },
  ...
  const compiler = new RevisionCompiler(registry.llm());
  const resolved = await compiler.resolve(options.instruction, storyboard, {
    organizationId,
    projectId: project.id,
    signal: context.signal,
  });

  const request = toRevisionRequest({
    projectId: project.id,
  ...
  await store.revisions.create(request, organizationId);
