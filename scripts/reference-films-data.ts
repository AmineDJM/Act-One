/**
 * The reference films, as data.
 *
 * Our own demonstration projects for fictional companies (see
 * reference-films.ts for why they are written by hand and labelled as such).
 * Kept apart from the script that renders them so other tools — the engine
 * parity measurement among them — can read the same films without rendering
 * the shop window as a side effect of importing it.
 */
